...
| `scripts/` (train, eval) | 003, 011（train_baseline_v0.py 改 Hydra entrypoint）, 027（`analyze_temporal_confounding.py` 一次性诊断脚本，不进 dvc pipeline，同 014/017 的 analyze_* 惯例） |
| `scripts/lo2-scripts/` | 001 |
| `docs/agent-docs/` | 001 |
| `docs/plans/` | 001 |
| `artifacts/` | 003, 024（`contract_ep2_0729*/` 手动构建，非 dvc repro 产出，无自动 .gitignore）, 027（`temporal_confounding/report.md` 诊断报告） |
| 数据采集协议 / benchmark 缺陷 | 017（采集审计，re-collection 暂缓）, 021（决定不重跑旧数据集，等重采）, 023（`KILLPOD` 目标即上报通道导致 trace 永久缺失）, 027（**固定时序模板导致时间混淆**：inject 起始 rel_pos std 仅 0.015，平凡规则即达 0.9656；单一 Normal case 位于 17 小时窗口起点；重采方案已记录但因 AnoMod 发论文而延期） |
...
| 标签粒度 / 评估口径 | 008（二档 `label_granularity` 引入）, 024（发现二档设计漏掉 service 档）, 025（补齐三档 + `label_target_observable`；**记录哪些历史数字作废**）, 027（pooled vs per-case 宏平均口径审计：本数据集两者 gap 仅 ±0.004，case 身份 shortcut 被证伪；但 per-case 宏平均仍应升为主指标 + 补 AUPRC 为并列主指标，理由见 entry） |
| `pyproject.toml` | 002 |
