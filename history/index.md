...
| [025](./entries/025-service-level-label-granularity.md) | 2026-08-22 | Bugfix + Experiment | service 级标签粒度补齐（`label_granularity` 二档→三档，新增 `"service"`：只有 `target_service` 时正样本收窄为 `is_anomaly & (service_name == target_service)`）+ 新列 `label_target_observable` 作为训练池吸收闸门（挡住 mysql/gateway 类不可观测 target 的 inject 行被整段吸进 One-Class 训练池）；`new_merge` 全量重跑证实 entry 024 的稀释假设：performance 档 AUROC 0.6137→0.9698、service 档 0.7351→0.9596、**endpoint 档不动**（0.8894→0.8820，关键对照），`Lv_P_CPU_preserve` 0.5428→0.9993；AUPRC **反而上升**（旧数字双双虚低，非"虚高被打回"）；六种机制均值全部改善、无一退化，L0/L2 均值持平（0.8887/0.8892，差值远小于 std）；RG 方差反而变大（softmax std 0.0352→0.1352，起草时曾因只有 seed42 而误判"RG 唯一退化"，已更正）；entry 014/016/018/022 的数字因 `endpoint_raw2`/`normal_v2` 永久丢失只能作废、无法重跑刷新 | `scripts/build_contract.py`, `src/contracts/contract_v0.py`, `tests/`（4 模块 + `nontarget_split_mini` fixture）, `artifacts/contract_new_merge_expanded/`, `artifacts/baseline_new_merge_*`, `data/new_merge`, `CLAUDE.md` |
| [026](./entries/026-content-length-clip-fix.md) | 2026-08-22 | Bugfix + Experiment | 修复 `content_length_mean` 归一化尺度爆炸（`order/refresh` fit-eval 电平错位，非长尾分布，log1p 已排除）：对该列归一化输出做对称 clip(±5)，排除退化 group；三方对比数字建立在 025 之前的标签口径上，与 master 合并后已过期，见 entry 内"数字失效声明"——待用户在新标签口径下重新跑一遍 `dvc_new_merge` 确认效果 | `scripts/build_contract.py`, `artifacts/baseline_new_merge_*/metrics.json` |
| [027](./entries/027-temporal-confounding-trivial-baseline.md) | 2026-08-25 | Experiment + Docs | 025+026 同时在位重跑六机制（**解除 026 的数字失效声明**，方向性结论在正确标签下成立；RG indep_sigmoid 完全塌陷，根因是退化列 z-score 爆炸压垮 sigmoid，本轮不修）+ **时间混淆诊断**：证伪"case 身份 shortcut"（pooled vs per-case 宏平均 gap 仅 ±0.004），但翻出两个更致命的问题——`rel_pos` 平凡基线（不看任何特征）per-case 宏平均 **0.9656**、零参数 z-score 基线 **0.9415**，双双打败最好的学习模型 DWF 0.9238；根因是采集用固定时序模板（inject 起始 rel_pos std 仅 0.015）+ `train_fit` 只有 1 个 Normal case（478 行）且位于 17 小时采集窗口的第 0 位；AnoMod 筹划发论文不便重采，修正工作转 TODO（P0 不依赖重采可立即做） | `scripts/analyze_temporal_confounding.py`, `artifacts/temporal_confounding/`, `artifacts/baseline_new_merge_*`, 研究方向/论文 framing, 数据采集协议 |
| [028](./entries/028-downstream-benchmark.md) | 2026-09-18 | Docs（补 PR #26 欠账） | 下游异常检测 benchmark（PR #26 合并时漏写 entry）：26 runs 口径下主网格 2 特征臂×2 池×3 检测器×4 seed=48 cell + 8 消融 = 56 run；expanded 池 per-case macro DeepSVDD 0.866→**0.957**、OCSVM 0.836→0.950、IF 0.733→0.814，三检测器方向一致；缺失值混淆消融（indicator-only 0.667、加 indicator 仅 +0.002）证明增益来自字段取值；平凡基线 expanded 池 rel_pos 0.579/zscore 0.766 远低于 headline ⇒ 返修数字安全，但 normal_only 池 rel_pos 0.800 打败全部学习模型故不入返修表；四个坑：commit message 旧数字（0.851→0.951 已过时）、macro 实际只覆盖 16 个 `Lv_E_HTTP*` case、承诺的 `_summary_agg.tsv` 不存在、与 contract 并行的第二套特征/切分实现 | `scripts/benchmark_downstream.py`, `scripts/run_benchmark_sweep.py`, `artifacts/downstream_benchmark/`, 研究方向/论文 framing, 标签粒度/评估口径 |
...
| 目录 / 主题 | 相关 entries |
|-------------|--------------|
| `CLAUDE.md` | 001, 002, 003, 004, 009, 011, 015, 016, 022, 023, 025 |
| `data/` 组织与 DVC | 002, 023（`endpoint_raw2` 物理删除后 merged_v2 链路无法重建；new_merge 剔除 `Lv_S_KILLPOD_gateway`）, 025（`new_ep1`/`normal_v2` 亦已永久丢失，四条历史管线无法从原始数据重建；`new_merge` 从异机备份恢复，cache/remote 均空，禁跑 `dvc checkout`） |
...
| `src/contracts/` | 003, 011（Contract v1 时序切分）, 014（`endpoint_id` 列、训练池扩容吸收 fault baseline 行）, 016（`split_fault_baseline_temporal` 两路时序切分）, 022（重命名为 `split_fault_phase_temporal`，新增 inject/recover 非目标行吸收）, 025（`contract_v0.py` 新增必需列 `label_target_observable` + `label_granularity` 枚举校验 + 正样本⊆`is_anomaly` 校验） |
| `scripts/` (train, eval) | 003, 011（train_baseline_v0.py 改 Hydra entrypoint）, 027（`analyze_temporal_confounding.py` 一次性诊断脚本，不进 dvc pipeline，同 014/017 的 analyze_* 惯例）, 028（`benchmark_downstream.py`/`run_benchmark_sweep.py` 下游检测器 benchmark，同样不挂 DVC；直读 `tt_fused_15s.csv`，与 contract 并行的第二套特征/切分实现） |
| `scripts/lo2-scripts/` | 001 |
| `docs/agent-docs/` | 001 |
| `docs/plans/` | 001 |
| `artifacts/` | 003, 024（`contract_ep2_0729*/` 手动构建，非 dvc repro 产出，无自动 .gitignore）, 027（`temporal_confounding/report.md` 诊断报告）, 028（`downstream_benchmark/` 56 run × {scores.parquet, metrics.json} + `summary.md`/`_summary.tsv`；**承诺的 `_summary_agg.tsv` 不存在**；产物随 PR #26 在 master，不含该 commit 的分支上工作区无文件，用 `git show 25cd20b:...` 取） |
| 数据采集协议 / benchmark 缺陷 | 017（采集审计，re-collection 暂缓）, 021（决定不重跑旧数据集，等重采）, 023（`KILLPOD` 目标即上报通道导致 trace 永久缺失）, 027（**固定时序模板导致时间混淆**：inject 起始 rel_pos std 仅 0.015，平凡规则即达 0.9656；单一 Normal case 位于 17 小时窗口起点；重采方案已记录但因 AnoMod 发论文而延期）, 028（时间混淆在下游 benchmark 的第二现场：expanded 池平凡基线 rel_pos 0.579/zscore 0.766 远低于 headline 0.957，返修数字安全；但 normal_only 池 rel_pos 0.800 打败该池全部学习模型，故该池不进返修表） |
...
| `tests/` | 002 (占位), 003 (契约/e2e), 009 (merged_v2_mini fixture + 归档排除/多 root 测试), 022 (`nontarget_split_mini` fixture，锁住 inject/recover 非目标行切分行为), 025（`nontarget_split_mini` 新增 `Lv_D_UNOBSERVABLE_mysql` 覆盖不可观测 target；删除一条断言旧 bug 的测试） |
| 标签粒度 / 评估口径 | 008（二档 `label_granularity` 引入）, 024（发现二档设计漏掉 service 档）, 025（补齐三档 + `label_target_observable`；**记录哪些历史数字作废**）, 027（pooled vs per-case 宏平均口径审计：本数据集两者 gap 仅 ±0.004，case 身份 shortcut 被证伪；但 per-case 宏平均仍应升为主指标 + 补 AUPRC 为并列主指标，理由见 entry）, 028（"26 runs" benchmark 的 per-case macro 实际只覆盖 16 个 `Lv_E_HTTP*` endpoint 级 case，其余 10 个 service/case 粒度 case 正样本为 0 不进 macro，引用时必须声明范围；commit message 数字 0.851→0.951 是 review 修复前旧值，以 `summary.md` 的 0.866→0.957 为准） |
| `pyproject.toml` | 002 |
| `history/` + `skills-local/` | 004 |
| `Makefile` | 002 (环境), 004 (install-skills) |
| 多模态融合（`src/fusion/`） | 005, 010, 012, 014（ReliabilityGatedFusion，偏离量门控路由）, 015（from_contract 钩子解耦）, 018（DeviationWeightedFusion，逐特征加权，零可学习参数，不达标）, 023（new_merge 上 L0/L1/L2/RG/DWF 全对比，softmax 坍缩未见复现）, 026（content_length clip 修复后 L0/DWF 恢复但 L1/RG 出现无关回退，疑似 SVDD 共享 embedding 连带效应；数字待新标签口径下重跑）, 027（**026 的重跑已完成，方向性结论成立**；但六种机制全部被零参数 z-score 基线打败，融合方向降格为消融维度，见横切主题） |
| 模型实现（`src/models/`） | 005 |
...
| `configs/contract/endpoint_to_service.yaml` | 006, 024（只映射 8 个客户端入口 service，mysql/gateway 类基础设施 target 结构性不可评估）, 025（定案：放行 `n_pos=0` + warning，**不**扩展映射表——扩表等于给 mysql 编造假 endpoint，污染特征侧比 null AUROC 更坏） |
| `src/data/normalization.py`（Normalizer） | 007, 013（零方差 group 除零放大）, 026（本身未改动，但 fit-eval 电平错位问题由其 min-max 设计放大，见 026 遗留 TODO） |
| `src/preprocessors/trace_preprocessor.py`（is_target_endpoint 传递） | 008 |
| `scripts/build_contract.py`（label_granularity/is_endpoint_anomaly 标签路由，`_attach_label_columns`） | 006, 008, 014（训练池扩容吸收 fault baseline 行）, 015（fit_endpoint_baseline_stats 开关取代 contract_version 判据）, 016（`_write_v1` 改按 fraction 时序切分 fault baseline）, 022（`_write_v1` 追加 inject/recover 非目标行两路切分）, 024（发现 case 级 `is_endpoint_anomaly` 未用 `target_service` 收窄，稀释 service_metric 判别力评估，未改代码）, 025（**落实修复**：三档 `label_granularity` + `label_target_observable` 吸收闸门 + eval_all 六段扩七段补行守恒漏洞）, 026（`content_length_mean` 归一化输出对称 clip，排除退化 group；数字待新标签口径下重跑） |
...
> ⚠️ **数字有效性声明（025 追加）**：entry **014 / 016 / 018 / 022** 发布的全部 AUROC/AUPRC/gate 权重数字**已作废且无法重跑刷新**——它们依赖 `merged_v2.yaml` = `anomod_v1` + `endpoint_raw2` + `normal_v2`，后两者已永久丢失（磁盘/DVC cache/remote 三处皆无）。它们同时还建立在 025 修掉的错误 service 级标签之上。下文引用这些 entry 的**机制性结论**（如坍缩形态、方差量级、方法学教训）仍然有效，但**任何具体数值只能当作"当时如此"的历史记录**，不可作为结论引用、不可与 025 之后的数字并列比较。当前唯一口径正确且可重建的数据集是 `data/new_merge`（见 025）。
...
- **标签精度分级**：数据集里不同数据源标签粒度不一致（case 级近似 vs endpoint 级精确）时，用 contract 层的显式列（如 `label_granularity`）标记精度，而不是靠数据源名字或已有的故障类型字段（如 `anomaly_level`）隐性判断——后者语义上不等价，未来数据源变化会静默失配（008）。**该列已于 025 从二档扩到三档**（`endpoint` / `service` / `case`）：008 的二分类心智模型"有 `target_endpoint` 就精确、没有就退到 case 级"漏掉了"无 `target_endpoint` 但有 `target_service`"这一档，而 `target_service` 从采集起就存在于 `case_metadata.json`（chaos 注入本身就是打向某个 service 的 pod）。缺的从来不是数据，是分档设计——**引入分级列时要穷举数据里实际存在的标识字段组合，而不是按"有没有最精确的那个字段"做二分**（024 发现 / 025 修复）
- **错误的标签会把强信号伪装成"特征没用"，且伪装得毫无破绽**：case 级 fallback 把整个 case 内所有 service 一视同仁标正，使纯 service 级特征（`service_metric`/`service_log`）的判别力被无关 service 的行稀释到接近随机——`Lv_P_CPU_preserve` 的 `cpu_usage_rate` 表观 AUROC 0.534、修正后 0.9993，6 个资源型故障 case 里 5 个原本落在 0.5~0.55 区间。**这种失效不产生任何异常信号**：不报错、不 NaN、不越界，只是让人得出"资源指标对资源型故障没有判别力"这个荒谬但看起来有数据支撑的结论。判据是**先怀疑评估口径再怀疑数据/预处理**：当某个特征在理论上必然响应的故障类型上表现为无信号时，优先核查正样本是怎么圈定的（024 的排查顺序值得复用：先用已有列手工重算验证根因，再动代码）。附带教训：修复后 AUPRC **上升**而非下降，说明被误标的正样本是纯噪声、同时压低了 AUROC 和 AUPRC——"正样本变少 → AUPRC 机械下降"的直觉在标签修正场景下不成立（025）
- **正样本率低不等于类别失衡 bug，要看成因**：025 修复后 `new_merge` 的 `eval_all` 正样本率降到 6.5%，低于 016 当年判定为失衡的 16%，但两者性质相反——016 是**负样本被掏空**（baseline 整段搬进训练池，属 bug），025 是**正样本收窄回真实身份**（负样本绝对数反而增加）。6.5% 更接近真实先验（一次注入只打 1 个 service，8 个 service 里 1 个受影响），且**没有任何 fraction 配置能把它"调回"旧的 19.7%**，因为旧数字里有 2197 行是误标的。核查类别比例时必须同时看正负样本的绝对数变化方向，只看比例会把"标签变准"误判成"新的失衡 bug"（025）
- **给筛选类判据加闸门时，必须检查被挡下的行有没有归宿**：025 给 `_write_v1` 的 inject 吸收侧加 `label_target_observable` 闸门后，不可观测 case 的 inject 行既不进训练池（被闸门挡住）、又不是正样本（`is_endpoint_anomaly` 全 False）、也不落在原有任何 eval_all 分段的条件里——2105 行会**静默消失**。捕获它靠的是既有的行守恒断言，不是任何显式测试。任何在"分流式"逻辑（每行必须恰好落进某一路）里新增过滤条件的改动，都要同步确认分流的完备性，并把守恒检查一起扩容（025）
- **测试可能在锁住 bug，修 bug 时要先判断测试本身的立场**：025 删掉的 `test_fallback_case_is_endpoint_anomaly_matches_is_anomaly` 断言 `is_endpoint_anomaly` 严格等于 `is_anomaly`，这正是被修掉的错误行为本身。看到测试失败的第一反应容易是"我改错了"，但对"修正既有错误语义"类改动，必须先分辨失败的测试是在保护正确契约还是在固化 bug——前者要改实现，后者要删测试并在 entry 里写明原因（025）
...
- **竞争性 softmax 门控比独立 sigmoid 更容易训练坍缩，但坍缩幅度与数据集相关，非普适结构性问题**：Reliability Gate Fusion 的路由消融显示，在 `merged_v2`/`endpoint_raw2` 上默认 `softmax`（权重和恒为1的二选一）训练后在全部 27 种故障类型上坍缩为几乎恒定的 `w_svc≈1.0`，而去掉竞争性归一化的 `independent_sigmoid` AUROC 更高（0.6598 vs 默认 0.6024，差距 0.0574）（014，**数字已作废，见上方声明**）。但在 `new_merge` 上重跑同一组消融（seed{1,2,3,42} 四组均值），softmax 与 indep_sigmoid 几乎持平（0.6575 vs 0.6570），且 indep_sigmoid 方差反而更大——说明坍缩是否发生、发生后收益多大，与具体数据集的特征分布/故障类型组合相关，换数据集不能默认沿用旧结论断言"indep_sigmoid 更优"（023，**该组数字亦基于修复前标签，025 重跑后 softmax 0.6015 / indep_sigmoid 0.6896，两者重新分开且 indep_sigmoid 更优**）。**025 追加的一条独立证据**：修复 service 级标签后 RG 是六种融合机制里**唯一退化**的（softmax 0.6510→0.6015），且其 endpoint 档从 0.7567 崩到 0.5307——这与坍缩机制自洽（`w_svc≈1.0` 时模型只看 service 分支，而 service 档标签变准、endpoint 档不变，只用 service 分支的模型在 endpoint 档上自然更差），是坍缩在更干净标签下暴露得更彻底，不是新问题。提示：门控输入维度低、监督信号只能通过下游 loss 间接传导（如 One-Class SVDD 无监督门控）时，softmax 的强制竞争约束可能让训练更容易早期卡进某一端主导的鞍点；同类机制设计如需门控随输入变化，优先验证独立参数化（sigmoid）而非竞争归一化（softmax）是否更不容易坍缩，坍缩现象本身需要专项训练过程可视化才能定位根因，不能仅凭最终权重分布下结论（014）
...
- **eval 集合的类别比例是否合理，需要独立于训练侵入之外单独核查**：门控坍缩的梯度只来自训练循环，`eval_all` 从不参与反向传播，"eval 类别失衡导致坍缩"这条因果链机制上不成立；但反过来，训练池扩容改 `eval_all` 摘除逻辑时，容易在无意中破坏 eval 集合本身的类别平衡（本例中负样本占比从约50%降到16%，且与 `CLAUDE.md` 既定的评估协议冲突），这是独立于训练动态之外必须单独核查的正确性问题，不能因为"不影响训练"就忽略（014）（016 落实修复：baseline 行改为按 fraction 时序切分而非整段搬移，eval 类别比例从 ~84:16 回升至约 24.62:75.38，虽未完全回到 50:50，但不再是接近全负样本被掏空的失衡状态）（022 进一步吸收 inject/recover 非目标行，实际比例未重新核算，见 022）（**025 补充：上述所有比例数字均基于已作废的错误标签，仅"必须单独核查"这条方法学结论有效；核查时还需区分"负样本被掏空"与"正样本收窄回真实身份"两种成因，见上方 025 条目**）
...
- **归一化特征的极端值排查要先验证是不是 fit-eval 电平错位，而不是默认当长尾分布处理**：`content_length_mean` 归一化后在 `order/refresh` 上出现 `|value|>10` 占比 0.80 的极端值，直觉上像长尾分布该用 `log1p` 压缩，但实测 `log1p` 反而更差——真实根因是 `Normalizer` 只在 Normal case 上 fit `[lo,hi]`，而该 endpoint 的 Normal case 采集时间早于故障批次、二者原始量纲本就落在不同电平（系统性采集时序漂移，非真实故障信号）。这类问题的正确诊断路径是先对比 fit 窗口与 eval 窗口的原始值分布是否处于不同电平，而不是看到极端值就假设是分布形状问题去套用变换；确认是电平错位后，能做的只有对归一化输出做边界 clip 封顶损害，无法用变换恢复干净信号（026）
- **单一特征列的下游修复可能通过 One-Class SVDD 共享 embedding 产生与该特征无语义关联的连带效应（归因假设，未验证）**：对 `content_length_mean` 一列、一个 endpoint 做对称 clip 后，`concat`(L0)/`deviation_weighted`(DWF) 按预期修复了 REPLACE 类回归且保留 PATCH 类增益，但 `independent_concat`(L1) 全面下滑、`reliability_gate`(RG, softmax) 的 DELAY 类（与 content_length 无语义关系的纯延迟故障）AUROC 崩至 0.435。可能的解释框架：SVDD 是跨全部 endpoint 共享的单一超球面嵌入，改变一个特征在一个 endpoint 上的训练期分布会通过共享 encoder 的梯度反传重新分配整个嵌入空间对各 endpoint/各故障类型的判别能力——这是 [[decision_fusion_gradient_cannot_learn_feature_relevance]]（融合梯度学不到特征相关性）在训练动态层面的另一种表现形式。本次只做单 seed（42）观察，未排除随机噪声，用户决定接受现状不做多 seed 复测（026）；如果后续要给这个连带效应下结论或依赖它做架构决策，必须先补 seed{1,2,3} 验证是否为系统性效应。注：本条记录的数字建立在 025 标签修复之前的口径上，**027 已在 025+026 同时在位的口径下重跑，方向性结论（L0/L2/DWF/RG-softmax 改善、L1 回退）成立**，026 的"数字失效声明"至此解除
- **任何 AUROC 都必须与"无学习基线"并列报告，否则数字无法解释**：027 在 `new_merge` 上实测两个平凡基线双双打败全部六种融合机制——① `rel_pos`（窗口在 case 内的相对时间位置，**不看任何特征**）per-case 宏平均 **0.9656**；② 每个 `(case,endpoint)` 用自己 baseline 阶段算 z-score 再取 L2 范数（**零参数、零训练、无需 Normal case**）**0.9415**；而最好的学习模型 DWF 只有 0.9238。这不是"模型不好"这么简单，它同时暴露了三件事：**(a) 数据集缺陷**——采集脚本用固定时序模板，inject 起始 `rel_pos` 跨 27 个 case 均值 0.586/**std 仅 0.015**，一条 `if rel_pos>0.58` 的规则即得 0.9656，这是 [Wu & Keogh 的 run-to-failure bias](https://wu.renjie.im/research/anomaly-benchmarks-are-flawed/) 在 case 内的版本，审稿人跑一次就能否掉整篇；**(b) 归一化参照系选错**——`train_fit` 只有 1 个 Normal case（478 行）且是 17 小时采集窗口里第 0 个采集的，全局统计量里混进了系统漂移，而 z-score 基线用 per-case 自适应参照系使漂移自动抵消（AIOps 标准做法，也正是 [Quo Vadis (ICML'24)](https://arxiv.org/html/2405.02678v1) 的 L2-norm 基线）；**(c) 架构没挣到复杂度**——单特征最高仅 0.71（`client_5xx_rate`）但 z-score 范数能到 0.94，说明信号是分散在多特征上的弱信号需要聚合，Deep SVDD 压进低维球心距离时反而损失了信息，这也解释了为何 L0 与门控始终不相上下（融合机制间差异被更上游的问题盖住）。**方法学结论：把平凡基线写进评估脚本作为强制对照，而不是靠记忆偶尔跑一次**；诊断"模型是不是在偷懒"时要区分"数据集有缺陷"与"模型在利用缺陷"——本例模型输入的 21 个特征里没有任何时间戳字段，且从特征回归 `rel_pos` 的 GroupKFold R² 仅 0.1026，两条证据合起来才能把责任正确归给数据集（027）
- **证伪一个 shortcut 假设的成本远低于修它，先证伪再动手**：用户提出"27 个 case 串行采集 → 基线偏移 → 模型靠学习『这是第几个 case』偷懒"这一假设，若成立则需要上 DANN/gradient-reversal 消除 embedding 里的 domain 信息（entry 027 调研已备好 LogTAD/LogDLR 的现成架构）。但只需比较 pooled AUROC 与 per-case 宏平均 AUROC 即可证伪——**per-case 口径的正负样本都取自同一 case，case 身份在该口径下数学上不可利用**，若模型真在用 case 身份，两个数字必然显著背离（[video AD 审计](https://arxiv.org/html/2608.11985) 实测出过排名反转）。实测 gap 全部落在 ±0.004，假设不成立，昂贵改造避免了。这个检验用已有 `scores.parquet` 零重训完成，**成本是几分钟**。同类模式：entry 017 也是先证伪 per-endpoint shortcut 才转向新 framing。诊断类改动的正确顺序是"最便宜的判别性实验优先"，不是"按假设的严重程度优先"（027）
- **一次性诊断脚本必须落盘，临时命令里的数字等于作废**：entry 014/016/018/022 的全部数字作废，直接原因是不可重跑（依赖已丢失的数据集）。027 的诊断数字先手工算、再固化进 `scripts/analyze_temporal_confounding.py`、再跑一遍验证与手算逐位一致，才写进 entry。项目已有此惯例（`analyze_gate_weights.py` / `analyze_modality_observability.py`），但惯例只覆盖"做完实验后补脚本"，027 补充的是**"数字进 entry 之前必须先有可复现路径"**这一前置条件（027）
