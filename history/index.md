...
| 标签粒度 / 评估口径 | 008（二档 `label_granularity` 引入）, 024（发现二档设计漏掉 service 档）, 025（补齐三档 + `label_target_observable`；**记录哪些历史数字作废**） |
| `pyproject.toml` | 002 |
| `history/` + `skills-local/` | 004 |
| `Makefile` | 002 (环境), 004 (install-skills) |
| 多模态融合（`src/fusion/`） | 005, 010, 012, 014（ReliabilityGatedFusion，偏离量门控路由）, 015（from_contract 钩子解耦）, 018（DeviationWeightedFusion，逐特征加权，零可学习参数，不达标）, 023（new_merge 上 L0/L1/L2/RG/DWF 全对比，softmax 坍缩未见复现）, 026（content_length clip 修复后 L0/DWF 恢复但 L1/RG 出现无关回退，疑似 SVDD 共享 embedding 连带效应；数字待新标签口径下重跑） |
| 模型实现（`src/models/`） | 005 |
