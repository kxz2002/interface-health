...
- **`Lv_S_KILLPOD_gateway`（n=221）AUROC 为 null，已确认非 bug**：新 `metrics.json` 里 `Normal`（n=236，按定义全负）、`by_anomaly_level.none`（n=236，同上）之外，`Lv_S_KILLPOD_gateway` 这个分层也是 `auroc: null`。专项核查发现：这个故障 case 本身在合并后的数据集里就没有任何 inject/recover 行（train.parquet 里该 case 的 39 行也全是 `phase==baseline`），即该 case 在 inject 注入这一步就没有产出可用的异常窗口——与 `fraction=0.2` 的切分比例无关，即便 `fraction` 取别的值，这个 case 的 eval 行仍会是 100% `phase==baseline`（全部 221 行）单一类别（全负），AUROC 数学上无定义。与 `Normal`/`none` 是同一类现象，不是本次切分逻辑引入的缺陷。
