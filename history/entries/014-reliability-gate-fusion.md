...
- **`_write_v1()` 训练池扩容建议做成 config 可控开关**：讨论中已提出方案（`expand_train_pool: bool`，默认 `false` 保持旧行为，新增 `configs/contract/v1_expanded_pool.yaml` 供 reliability_gate 专属实验用），尚未实现。实现后可同时解决"能否跟旧实验比"和"能否分别研究扩容前/后的坍缩动态"两个目的。
