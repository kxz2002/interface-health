...
- **v0 与 v1 的 metrics 不可直接比较**：v0 `train ⊆ eval_all`（评估集包含训练时见过的 Normal 行），v1 修复了这个重叠。真实数据实测：v0 AUROC=0.634 / AUPRC=0.303（14717 样本），v1 AUROC=0.566 / AUPRC=0.289（13632 样本，因 `eval_all` 只保留 holdout Normal + 全部异常行，样本数减少）。v1 更低是泄漏修复后的诚实回落，不代表模型变差——后续写论文/报告严禁把两者放在同一张表里当作"同一实验的两次跑"，必须注明协议版本。
