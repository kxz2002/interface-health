...
- **L3（全局标量门控）是否值得做**：optimizer bug 修复后，本轮 L2 相对 L1 有正向且比修复前更明显的提升（AUROC +0.0325，AUPRC +0.0160），相对 L0 则是 L1 略降、L2 提升更清楚（L0 AUROC=0.6169 高于 L1 的 0.6065，但低于 L2 的 0.6390，L2 相对 L0 已有 +0.0221 AUROC）。三组数字说明"引入非线性容量"（L0→L1）本身没有带来提升，而"引入门控"（L1→L2）才有，且修复训练 bug 后这个增量比之前（融合层未训练时）测出来的更大——说明门控机制在被正确训练之后确实能学到有意义的调制，不是训练误差范围内的噪声。是否继续做 L3 取决于下一轮讨论：如果目标是把消融矩阵做完整（L0-L3 全跑一遍是原计划），可以顺手补上；如果目标已经转向"设计真正的新融合模型"，L3 可能不再必要，直接拿 L1/L2 的数据去支撑新设计的动机即可。
