...
- **`expand_train_pool` 开关会改变 eval_all 行数，不同取值的 AUROC 不可直接横向比较**：`false`（`contract_v1`）eval_all=13632 行，`true`（`contract_v1_expanded`）eval_all=12683 行（issue #16 修复后的数字；修复前为 8221 行，见下一条）——后者摘除了被吸收进训练池的故障 baseline 行。L0/L1/L2 与 RG 分别固定用其中一种取值（见上方 Commands 小节），不要把两者的 AUROC 直接摆在一张表里比大小，除非明确注明各自的 eval 样本数和口径差异。`EndpointBaselineStats`/`Normalizer` 的 fit 范围不受该开关影响，始终锁定纯 Normal 的 `train_fit`
- **`expand_train_pool=true` 后 eval_all 类别比例由 `fault_baseline_train_fraction` 控制（issue #16 已修复）**：修复前 baseline 阶段行整段进训练池、整段摘出 eval，导致 eval_all 正负比崩到 ~84:16。现在故障 baseline 行按时间窗时序切分（`src/contracts/split_fault_baseline.py`）：最早 `fault_baseline_train_fraction` 比例（默认 0.2）的窗口进训练池，其余留在 eval_all。真实 29-case 数据上实测正负比回升至约 24.62%:75.38%（负样本占比从 ~16% 回升到 ~75%，未完全回到 50:50——inject/recover 行不受影响，且 fraction 只挪 20% 的 baseline 行）。`train.sample_id ∩ eval_all.sample_id == ∅` 不变量由整窗切分保证。后续扩充 Normal 数据集后可调该 fraction。跨不同 fraction 取值比较 AUROC/AUPRC 时仍需注明各自的类别比例。见 history/entries/016
